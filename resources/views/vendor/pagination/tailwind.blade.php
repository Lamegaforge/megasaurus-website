@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between mt-6 px-4 md:justify-center">
        <div class="flex justify-between items-center flex-grow md:hidden">
            ...
            <p class="flex-grow text-center text-sm text-white">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span class="block px-4 py-2 text-indigo-400 focus:text-indigo-400">{{ $element }}</span>
                            </span>
                        @endif
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="block px-4 py-2 text-indigo-400 font-bold">{{ $page }}</span>
                                    </span>
                                @else
                                    <a
                                        href="{{ $url }}"
                                        class="px-4 py-2 text-white hover:text-indigo-400 focus:text-indigo-400 transition-colors ease-in-out duration-200"
